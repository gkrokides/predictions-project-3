...
def predictions(request):
    ...
    if request.method == "POST":
        ...
        if lg_name == "select_league" or lg_name == '' or not lg_name or lg_name is None or period_end == 'None' or period_end == '':
            ...
            home_wins_total_perc = format(float(home_wins_total) / games_played_inc_po, "0.00%")
            away_wins_total = Game.objects.total_season_away_wins(seasonid)
            away_wins_total_perc = format(float(away_wins_total) / games_played_inc_po, "0.00%")
            draws_total = Game.objects.total_season_draws(seasonid)
            draws_total_perc = format(float(draws_total) / games_played_inc_po, "0.00%")
            season_goals = Game.objects.total_season_goals(seasonid)
            goals_p_game = float(season_goals) / games_played_inc_po
            bts = Game.objects.season_both_teams_scored(seasonid)
            bts_perc = format(float(bts) / games_played_inc_po, "0.00%")
    ...
    if request.method == "GET":
        ...
        while attempt_num < MAX_RETRIES:
            ...
            if r.status_code == 200:
                ...
                dataJson = json.dumps(data, sort_keys=True, indent=4)
                dataDict = json.loads(dataJson)
                smData = dataDict['data']
                live_fixtures = [x['id'] for x in smData]
                for g in upcoming_predictions:
                    currentMatch_id = g.pk
                    minute = ''
                    score = '-'

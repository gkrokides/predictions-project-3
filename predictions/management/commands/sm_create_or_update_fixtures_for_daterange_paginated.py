    ...
    def handle(self, *args, **options):
        ...
        for x in allFixtures:
            if FixtureSM.objects.filter(fixture_id=x['fixture_id']).exists():
                ...
                self.stdout.write(self.style.SUCCESS('"\r%%%s" updated ' % (100 * float(cntUpdated) / float(len(allFixtures)))), ending='\r')
                cntUpdated += 1
                ...
                self.stdout.write(self.style.SUCCESS('"\r%%%s" created ' % (100 * float(cntCreated) / float(len(allFixtures)))), ending='\r')

...
# This function returns a dict of the selected odds. If it doesn't find odds from the
# bookies set below, it chooses the first available odd set from the pool of data.
def smGetOddsForFixture(mid, fid):

	market_id = mid #enter the market for which you want the odds
	fixtureId = fid #enter the fixture id
	...
	bookmakerIDs = [bookie['id'] for bookie in resultDict[0]['bookmaker']['data']]
	try:
